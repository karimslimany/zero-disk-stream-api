...
function startEngine(infoHash, magnet) {
    ...
    try {
        ...
        // *** تجربة تحكّمية (بعد رصد نظير واحد فقط بطيء جداً رغم نجاح مرحلة الميتاداتا) ***
        // عطّلنا DHT سابقاً بافتراض عام أن UDP محجوب كلياً على Render. هذا افتراض معقول
        // لكن لم يُختبر مباشرة: كثير من شبكات NAT "بلا دعم UDP رسمي" (أي بلا توجيه UDP
        // وارد لبورت عشوائي من الإنترنت) لا تزال تسمح بنمط طلب/رد UDP الصادر من داخل
        // الحاوية نفسها (وهو بالضبط ما يحتاجه DHT: طلب صادر ثم رد يعود لنفس الاتصال
        // المفتوح، لا اتصالاً وارداً جديداً غير مرتبط). لذا نعيد تفعيله كتجربة مع إبقاء
        // التراكرز كما هي، ونراقب [Swarm Monitor]:
        //   - لو ارتفع عدد "نظراء متصلون" فوق 1 خلال أول 30-40 ثانية → DHT يعمل فعلاً هنا،
        //     أبقه مفعّلاً.
        //   - لو عادت رسالة "No seeders / trackers unreachable" الشاملة على كل التورنتات
        //     كما حدث في أول اختبار → أرجعه إلى false فوراً (كان هذا هو الإصلاح الأصلي
        //     لتلك المشكلة تحديداً).
